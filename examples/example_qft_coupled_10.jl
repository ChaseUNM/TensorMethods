...
quandary_data_path = joinpath(@__DIR__, "quandary_data")
rho_re = vec(readdlm(joinpath(quandary_data_path, "quandary_re_coupled_10.dat")))
rho_im = vec(readdlm(joinpath(quandary_data_path, "quandary_im_coupled_10.dat")))
quandary_ans = rho_re + im*rho_im

#Determine how many different truncation parameters 
pts = 20
pts_range = LinRange(-15, -1, pts)
cutoff_list = 10.0 .^pts_range
...
#Set up initial states for MPS and Tucker-tensor
q_state = fill(0, N)
...
#Create hamiltonian as MPO for TDVP and local operators for BUG
...
#Count entries at end of evolution for each truncation parameter
if plot_tdvp == true 
...
#Create plots
if plot_tdvp && plot_bug
    cutoff_plot = plot(cutoff_list, [err_list_tdvp, err_list_bug], label = ["TDVP2 Err" "BUG Err"], xscale =:log10, yscale=:log10, legend =:topleft, dpi = 250, xticks = [10^-16, 10^-15, 10^-14, 10^-13, 10^-12, 10^-11, 10^-10, 10^-9, 10^-8, 10^-7, 10^-6, 10^-5, 10^-4, 10^-3, 10^-2, 10^-1], xlabel = "SVD Truncation Parameter(ε)")
...
elseif plot_tdvp
    cutoff_plot = plot(cutoff_list, err_list_tdvp, label = "TDVP2 Err", xscale =:log10, yscale=:log10, legend =:topleft, dpi = 250, xticks = [10^-16, 10^-15, 10^-14, 10^-13, 10^-12, 10^-11, 10^-10, 10^-9, 10^-8, 10^-7, 10^-6, 10^-5, 10^-4, 10^-3, 10^-2, 10^-1], xlabel = "SVD Truncation Parameter(ε)")
...
elseif plot_bug
    cutoff_plot = plot(cutoff_list, err_list_bug, label = "BUG Err", xscale =:log10, yscale=:log10, legend =:topleft, dpi = 250, xticks = [10^-16, 10^-15, 10^-14, 10^-13, 10^-12, 10^-11, 10^-10, 10^-9, 10^-8, 10^-7, 10^-6, 10^-5, 10^-4, 10^-3, 10^-2, 10^-1], xlabel = "SVD Truncation Parameter(ε)")
