...
struct bcparams
    ...
    # New constructor to allow defining number of symmetric Hamiltonian terms
    function bcparams(T::Float64, D1::Int64, Ncoupled::Int64, Nunc::Int64, omega::Vector{Vector{Float64}}, pcof::Array{Float64,1})
        dtknot = T/(D1 -2)
        tcenter = dtknot.*(collect(1:D1) .- 1.5)
        # Nfreq = size(omega,2)
...
#Simplified constructor (assumes no uncoupled terms)
function bcparams(T::Float64, D1::Int64, omega::Vector{Vector{Float64}}, pcof::Array{Float64,1})
    dtknot = T/(D1 -2)
    tcenter = dtknot.*(collect(1:D1) .- 1.5)
    Ncoupled = size(omega,1) # should check that Ncoupled >=1
    Nfreq = [length(omega[i]) for i in 1:length(omega)]
...
function s_op_reverse(op, j, N)
    ...
    elseif j == N
        ...
        I1 = Matrix(I, 2^(N - j), 2^(N - j))
        I2 = Matrix(I, 2^(j - 1), 2^(j - 1))
        return kron(I1, op, I2)
    end
end
...
function xxx_mpo_scaled(N, sites, J, g)
    os = OpSum()
    for i = 1:N - 1
        os -= J, "Sz",i, "Sz", i + 1
    end
...
function graziani_H(N, J, U, hj_list, hp_list)
    ...
    for i =  1:N
        # println("i: $i")
        if i != N  
            H .+= -J*(s_op_reverse(sx, i, N)*s_op_reverse(sx, i + 1, N) + s_op_reverse(sy, i, N)*s_op_reverse(sy, i + 1, N))
            H .+= U*s_op_reverse(sz, i, N)*s_op_reverse(sz, i + 1, N)
            # println("H $i")
            # display(U*s_op(sz, i, N)*s_op(sz, i + 1, N))
        end
        H .+= hj_list[i]*s_op_reverse(sz, i, N)
        H .+= hp_list[i]*s_op_reverse(sx, i, N)
    end
    return H
end
...
#Create new MPO with control pulses
function total_MPO(N::Int64, sites::Vector{Index{Int64}}, transition_freq::AbstractArray, rot_freq::AbstractArray, self_kerr::AbstractArray, zz::AbstractArray, dipole::AbstractArray, bc_params::bcparams, t::Float64)
    os = OpSum()
    for i = 1:N
...
#         H .+= (transition_freq[i] - rot_freq[i])*s_op_general_reverse(a'*a, i, N, N_levels[i])
#         H .-= 0.5*self_kerr[i]*s_op_general_reverse(a'*a'*a*a, i, N, N_levels[i])
#         p = bcarrier2(t, bc_params, 2*(i - 1))
#         q = bcarrier2(t, bc_params, 2*(i - 1) + 1)
#         control_a = p + im*q 
#         control_adag = p - im*q
#         H .+= control_a*s_op_general_reverse(a, i, N, N_levels[i]) + control_adag*s_op_general_reverse(a, i, N, N_levels[i])
#         if i != N 
#             for j = i + 1: N
#                 #zz-coupling interaction
#                 H .-= zz[i,j]*s_op_general_reverse(a'*a, i, N, N_levels[i])*s_op_general_reverse(a'*a, j, N, N_levels[j])
#                 #dipole-dipole interaction
#                 H .+= dipole[i,j]*s_op_general_reverse(a', i, N, N_levels[i])*s_op_general_reverse(a, j, N, N_levels[j])
#                 H .+= dipole[i,j]*s_op_general_reverse(a, i, N, N_levels[i])*s_op_general_reverse(a', j, N, N_levels[j])
#             end
#         end
#     end
...
##########################################################################################
##########################################################################################
#Construction of Hamiltonians to use with Tucker-Tensors
...
#xxx hamiltonian with operators scaled by 0.5
function ops_xxx_scaled(N, J, g)
    H_single_site = Vector{Tuple{AbstractMatrix, Int64}}()
    H_two_site = Vector{Tuple{AbstractMatrix, Int64, AbstractMatrix, Int64}}()
    for i in 1:N 
        tup = (-g*0.5*J*sx, i)
