...
cutoff_arr = 10 .^ -LinRange(5,10.1,30)
ans_arr = []
bd_arr = []
...
diff_plot = plot(cutoff_arr[1:length(cutoff_arr) - 1], diff_arr, yscale =:log10, xscale =:log10, xlabel = "εₙ (SVD Truncation Parameter)", label = "||y(εₙ) - y(εₙ₊₁)||", title = "XXX Heisenberg, N = $N qubits, J = $J, g = $g", yticks = [10^-1, 10^-3, 10^-5, 10^-7, 10^-9, 10^-11, 10^-13], xticks = [10^-5, 10^-6, 10^-7, 10^-8, 10^-9, 10^-10], dpi = 250, legend =:topleft)
